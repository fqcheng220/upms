...
public class ProductSkuAttrValueController {
     ...
     * @return
     */
    @RequestMapping(value = UrlPathConstants.PRODUCT_SKU_ATTRVALUE_UPDATE_ENHANCED, method = RequestMethod.POST)
    @ResponseBody
    @CrossOrigin
    public BaseResponseBody updateEnhanced(@RequestBody BaseRequestUpdateBody<ProductSkuDto> requestBody) {
        try {
            //如何直接获取path??
            RequestHandler.handle(UrlPathConstants.PRODUCT_SKU_ATTRVALUE_UPDATE_ENHANCED, requestBody);
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ProductSkuAttrValueUpdateResult result = mService.updateEnhanced(requestBody.mEntity);
        ...
        return new BaseResponseBody<>().setmStatusCode(ResponseConstants.STATUS_SUC)
                .setmMsg(String.format(ResponseConstants.MSG_SUC_UPDATE_FORMAT, String.format("商品SKU属性值（加强版） 删除%s 更新%s 新增%s ",deleteResult,updateResult,insertResult)))
